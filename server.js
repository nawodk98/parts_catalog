...
const server = app.listen(PORT, '0.0.0.0', () => {
    ...
    // Automatically open the website in kiosk mode (full screen, no window) if possible
    const { exec } = require('child_process');
    const startCmd = process.platform === 'win32' 
        ? `start msedge --kiosk ${url} || start chrome --kiosk ${url} || start ${url}` 
        : process.platform === 'darwin' 
            ? `open -a "Google Chrome" --args --kiosk ${url} || open ${url}` 
            : `google-chrome --kiosk ${url} || chromium-browser --kiosk ${url} || firefox --kiosk ${url} || xdg-open ${url}`;
    exec(startCmd, (err) => {
