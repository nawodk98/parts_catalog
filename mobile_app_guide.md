# Parts Catalog Mobile App (React Native)

To build the Android/iOS version of your Parts Catalog (view-only mode), we will use **Expo**, which is the fastest way to get a React Native app running on your actual device or emulator.

## Step 1: Create the Project
Since we want to make this app in a **separate folder**, open your terminal in your `d:\cat\parts_catalog\` folder and run:

```bash
npx --yes create-expo-app PartsMobile --template blank
```

*(This will create a brand new folder called `PartsMobile` specifically for this app).*

## Step 2: Replace `App.js`
Once the project is created, open the newly created `PartsMobile/App.js` file and replace **all** of its contents with the code below. 

This code provides a beautiful dark-mode design that replicates your `index.html` frontend, complete with "Genuine" and "OEM" badging and both Search tabs!
...
## Step 3: Run the App
Open your terminal inside the **new `PartsMobile` folder** and start the packager:
```bash
npx expo start
```
